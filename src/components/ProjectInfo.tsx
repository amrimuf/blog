    ...
    return (
        <div className='w-full md:w-10/12 flex flex-col space-y-2 py-6'>
            <div className="flex flex-row gap-4 items-center">
                <h1 className='font-bold text-xl sm:text-3xl text-neutral-900 dark:text-neutral-100'>{projectInfo.title}</h1>
            ...
            <div className="flex flex-wrap justify-start gap-2 text-neutral-900 dark:text-neutral-100">
                {projectInfo.tags.map((tag:any,index:any) => <div key={index} className="ring-1 ring-lime-500 italic px-2 rounded-full ">{tag.name}</div>)}
            </div>
        </div>
    );
}