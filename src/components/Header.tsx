      ...
      <div className="container max-w-4xl mx-auto flex items-center justify-between px-4 lg:px-0">
        ...
        <div className="space-x-8 hidden md:flex capitalize">
          {navPaths.map((navPath, index) => 
              <Link key={index} href={ `/${navPath}`} className={router.asPath.split('/')[1] === navPath || router.asPath.split('/').length == 1 ? 'font-bold' : 'font-normal'}>
                {navPath !== '' ? navPath : 'home'}
                <BsArrowDown className={`${router.asPath.split('/')[1] === navPath || router.asPath.split('/').length == 1 ? 'stroke-[1.5px] inline-block text-base text-lime-500 ml-[3px]' : 'hidden'}`}/>
              </Link>           
          )}
        </div>
