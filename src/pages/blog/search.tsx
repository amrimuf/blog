...
export async function getServerSideProps({ query, res}:GetServerSidePropsContext) {
    ...
    const allPosts = (await getPosts()) || [];
    allPosts
        .filter((post:{title:string,headline:string,category:string,topics:Topic[]}) => {
            // exclude non-alphanumeric (space, etc)
            const queryWords = q.toLowerCase().split(/\W+/).filter(Boolean);

            return (
                queryWords.some(query =>
                    post.title.toLowerCase().includes(query) ||
                    post.headline.toLowerCase().includes(query) ||
                    post.category.toLowerCase().includes(query) ||
                    post.topics.some(topic => topic.name.toLowerCase().includes(query))
                )
            );
        })
